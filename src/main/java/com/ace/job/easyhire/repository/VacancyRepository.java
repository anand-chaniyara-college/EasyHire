...
import com.ace.job.easyhire.dto.ChartDTO;
...
public interface VacancyRepository extends JpaRepository<Vacancy, Long>, DataTablesRepository<Vacancy, Long> {
	...
	@Query("SELECT new com.ace.job.easyhire.dto.VacancyForInterviewDTO(v.id, v.position, v.department,v.createdDate,v.dueDate) FROM Vacancy v")
	...
	@Query("SELECT new com.ace.job.easyhire.dto.ChartDTO(s.name,COUNT(c.id)) " + "FROM Vacancy v "
	...
	@Query("SELECT new com.ace.job.easyhire.dto.ChartDTO(CONCAT('Interview',' stage ',CAST(i.stage AS string),' ', ci.status), COUNT(c.id)) FROM Vacancy v "
	...
	// to get vacancyList for interview add
	@Query("SELECT new com.ace.job.easyhire.dto.VacancyForInterviewDTO(v.id, v.position, v.department,v.createdDate,v.dueDate) FROM Vacancy v WHERE v.createdDate >= :threeMonthsAgo")
	List<VacancyForInterviewDTO> getVacanciesForInterview(LocalDate threeMonthsAgo);

	// to get vacancyList for interview add
	@Query("SELECT new com.ace.job.easyhire.dto.VacancyForInterviewDTO(v.id, v.position, v.department,v.createdDate,v.dueDate) FROM Vacancy v WHERE v.createdDate >= :threeMonthsAgo OR v.id = :vacancyId")
